...
abstract class PDOConnection extends Connection
{
    ...
    public function getPDOStatement(string $sql, array $bind = [], bool $master = false, bool $procedure = false): PDOStatement
    {
        try {
            ...
            $note_array = array();
            $note = '';
            if(!empty($_SERVER['SCRIPT_FILENAME']))$note_array[] = "名称：".$_SERVER['SCRIPT_FILENAME'];
            if(!empty($_SERVER['REMOTE_ADDR']))$note_array[] = "路径：".$_SERVER['REMOTE_ADDR'];
            if(!empty($_SERVER['REQUEST_URI']))$note_array[] = "请求：".$_SERVER['REQUEST_URI'];
            //if(isset($GLOBALS['_SERVER']['PWD']))$note_array[] = "：".$GLOBALS['_SERVER']['PWD'];
            if(isset($GLOBALS['argv']) && !empty($GLOBALS['argv']) && is_array($GLOBALS['argv']))$note_array[] = "参数：".implode(" ", $GLOBALS['argv']);
            //不为空
            if(!empty($note_array))$note = implode("; ", $note_array);
            $sql = $sql."  /** ".$note." **/";
            //pod 预处理sql 优化结束
